...
def run_diamond():
    ...
    command="python "+script_directory+"Diamond_energy.py '"+molinchi+"' "+molname
    os.system(command)
...
def mini4diamond():
    ...
    while num < file_count:
        with open(script_directory+minifiles_name[num]+"_minimization.com", "w+") as f:
            ...
            f.write(" SOLV       3      5      0      0     0.0000     0.0000     0.0000     0.0000\n")
            f.write(" BDCO       0      0      0      0    41.5692 99999.0000     0.0000     0.0000\n")
            f.write(" CRMS       0      0      0      0     0.0000     0.5000     0.0000     2.0000\n")
            f.write(" BGIN       0      0      0      0     0.0000     0.0000     0.0000     0.0000\n")
            f.write(" READ       0      0      0      0     0.0000     0.0000     0.0000     0.0000\n")
            f.write(" CONV       2      0      0      0     0.0010     0.0000     0.0000     0.0000\n")
            f.write(" MINI       1      0    500      0     0.0000     0.0000     0.0000     0.0000\n")
            f.write(" END        0      0      0      0     0.0000     0.0000     0.0000     0.0000\n")
        f.close()
        command="bmin -WAIT {0}{1}_{2}_diamondH_minimization".format(script_directory, molname, minifiles_name[num].split("_")[1])
        os.system(command)
        num+=1
...
def frequency_test():
    ...
    while num < file_count:
        ...
        f.close()
        command="bmin -WAIT {0}{1}_{2}_diamondH_fretest".format(script_directory, molname, minifiles_name[num].split("_")[1])
        os.system(command)
        num+=1
...
molinchifile = [f for f in os.listdir(script_path) if f.endswith('.txt')]
print(molinchifile)
...
file_outputmae=files_in_directory(dir_path, minimolfile_extension)
minifiles_name=[]
