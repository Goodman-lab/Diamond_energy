...
def sort_runningtime():
    def custom_sort(item):
        # Extract the 'c' prefix and numeric value, if possible
        match = re.match(r'c(\d+)([a-zA-Z]*)', item)
        if match:
            num, suffix = match.groups()
            return (int(num), suffix)
        return (0, item)

    # Read the Diamond_runningtime.csv file
    df_runningtime = pd.read_csv("Diamond_runningtime.csv")

    # Convert the sorting column to string (if it's not already)
    df_runningtime[df_runningtime.columns[0]] = df_runningtime[df_runningtime.columns[0]].astype(str)

    # Sort the DataFrame based on custom sorting of the first column
    df_runningtime = df_runningtime.sort_values(by=df_runningtime.columns[0], key=lambda x: x.map(custom_sort))

    # Save the sorted DataFrame back to Diamond_runningtime.csv
    df_runningtime.to_csv("Diamond_runningtime_sorted.csv", index=False)

    print("File has been sorted.")

def average_time():
    # Read the existing CSV file into a DataFrame
    df = pd.read_csv("Diamond_runningtime.csv")

    # Check that the required columns exist in the DataFrame
    if 'Total_Running_Time' in df.columns and 'Total_Structures_Found' in df.columns:

        # Calculate the average time per conformer and store it in a new column
        df['Avetime/conf(s)'] = df['Total_Running_Time'] / df['Total_Structures_Found']

        # Handle cases where the total number of structures found is zero to avoid division by zero
        df['Avetime/conf(s)'].replace([float('inf'), float('-inf')], float('nan'), inplace=True)

        # Save the DataFrame back to the same CSV file, overwriting it
        df.to_csv("Diamond_runningtime.csv", index=False)

        print("New column 'Avetime/conf(s)' has been added.")

    else:
        print("Required columns 'Total_Running_Time' and 'Total_Structures_Found' not found in the CSV file.")
...
# Initialize the list to hold dataframes
dfs = []
...
for prop in files_and_properties:
    ...
    if os.path.exists(filename):
        ...
        if 'Total_Running_Time' in df.columns:
            print(f"Found 'Total_Running_Time' in {filename}")
            df_filtered = df[['Molecule', 'Total_Running_Time']].dropna()
...
if len(dfs) > 0:
    df_merged = dfs[0]
    for df in dfs[1:]:
        df_merged = pd.merge(df_merged, df, on='Molecule', how='inner', suffixes=('', '_dup'))

    # Plotting
    plt.figure(figsize=(15, 10))

    for prop in files_and_properties:
        label_column = 'Total_Running_Time' if prop['label'] == 'Diamond_MM3' else 'Total_Running_Time_dup'

        if label_column in df_merged.columns:
            plt.scatter(df_merged['Molecule'], df_merged[label_column],
                        color=prop['color'], label=prop['label'], marker=prop['marker'], s=prop['markersize'])

    plt.title("Total Running Time of Diamond Energy and Other Methods")
